...
-- create server common_oltp_server foreign data wrapper postgres_fdw options(dbname 'common_oltp');
-- create user mapping for current_user server common_oltp_server;
...
-- drop function if exists millis_to_time(decimal);
...
create view jiveuser (userid,username,passwordhash,name,namevisible,email,emailvisible,creationdate,modificationdate) as
  select x0.user_id ,x0.handle ,'' ,'' ,0 ,x1.address,0 ,0 ,0 from common_oltp."user" x0 ,common_oltp."email" x1 where ((x0.user_id = x1.user_id ) AND  (x1.primary_ind = 1. ) ) ;
...
create function trig_jivemessage_insert_event() returns trigger as $$
begin
    perform setval('corona_event_corona_event_id_seq', max(corona_event_id)) from corona_event;
     insert into common_oltp.corona_event(corona_event_type_id,user_id)  values (2 ,nw.userid);
	return new;
end;
$$ language plpgsql;
